...
sentencia_simple:    IDENTIF '=' expresion      { sprintf (temp, "(%s %s !)", $3.code, $1.code) ; 
                                           $$.code = gen_code (temp) ; }
            ...
            | '(' PRINT STRING ')' { sprintf (temp, ".\"%s\"\n", $3.code);
                                    $$.code = gen_code(temp);}
            |  '(' PRINC IDENTIF ')' { sprintf (temp, "%s @ .\n", $3.code);
                                    $$.code = gen_code(temp);}
            ;

sentencia_bloque:   '(' DEFUN IDENTIF '('parametro')' bloque_codigo ')' { sprintf(temp, ": %s \n%s;", $3.code, $7.code);
                                                        $$.code = gen_code(temp); }

                    | '(' DEFUN MAIN '(' ')' bloque_codigo ')' { sprintf(temp, ": main \n %s ;", $6.code);
                                                $$.code = gen_code(temp); }
                    | '(' LOOP WHILE '(' expresion ')' DO bloque_codigo')' {sprintf (temp, "begin %s while \n %s repeat", $5.code, $8.code);
                                                                            $$.code = gen_code(temp);
                                                                            }
                    | if_sin_else {$$.code = $1.code;}
                    | if_con_else {$$.code = $1.code;}
                    ;

if_sin_else:        '(' IF '(' expresion ')' '(' PROGN bloque_codigo ')'  ')' {sprintf (temp, "%s if \n%s\n then", $4.code, $8.code);
                                                                                $$.code = gen_code(temp);
                                                                                }
                    ;


if_con_else:        '(' IF '(' expresion ')' '(' PROGN bloque_codigo ')' '(' PROGN bloque_codigo ')' ')' {sprintf (temp, "%s if \n%s\n else \n%s\n then ", $4.code, $8.code, $12.code);
                                                                                    $$.code = gen_code(temp);
                                                                                    }
                    ;

bloque_codigo:       sentencia { $$ = $1; }
                    | bloque_codigo sentencia {sprintf(temp, "%s %s", $1.code, $2.code);
                                                $$.code = gen_code(temp);
                                                }
                    ;
